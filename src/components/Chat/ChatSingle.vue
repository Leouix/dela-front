...
async function toSendMessage(e) {
    ...
    if (input.value.trim()) {
        store.addMessage(input.value, e.target.chat_id)
        store.markUnread(e.target.chat_id)
...
onMounted(() => {
    store.getChatMessages(props.selectedJobApplication.chat_id);
...
<template>
    ...
    <div class="chat-single">
            ...
            >
                <div class="chat-item">
                    ...
                    <div class="chat-item-text"
                         v-html="store.linkify(msg.message)"
                    ></div>
        ...
        <form ref="formRef" id="chat-ai-form"  @submit.prevent="toSendMessage">
